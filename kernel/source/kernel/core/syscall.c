...
// 系统调用表, 系统调用号作为索引, 通过数组下标(系统调用号)迅速找到系统调用处理函数
...
void do_handler_syscall(syscall_frame_t * frame)
{
    ...
    if (frame->func_id < sizeof(sys_table) / sizeof(sys_table[0]))
    {
        ...
        if (handler)
        {
            ...
            frame->eax = ret;  // 设置系统调用的返回值，由eax传递给应用层
            return;
        }
    }
 ...
 * 处理系统调用, 该函数由中断/异常门调用
 */
void do_handler_syscall_irq(exception_frame_t * frame)
{
    // 将寄存器中传入的参数按顺序取出来
    uint32_t func_id = frame->eax;
    ...
    if (func_id < sizeof(sys_table) / sizeof(sys_table[0]))
    {
        ...
        if (handler)
        {
            int ret = handler(frame->ebx, frame->ecx, frame->edx, frame->esi);
            frame->eax = ret;  // 设置系统调用的返回值，由eax传递给应用层
            return;
        }
    }
