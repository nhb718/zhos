 ...
 *   /home -- fat16文件系统
 *   /other -- ntfs分区
 *   解析路径来确定设备类型, 根据路径识别不同的文件系统
 *
 * 2. 当新增设备后, 如何尽可能少的更改文件系统接口层代码
 *   fs_opt_t结构体中定义通用的操作接口: mount, unmount, open, read, write, close...
 *   devfs模块实现对应的接口: devfs_mount, devfs_open ...
 *   fatfs模块实现对应的接口: fatfs_mount, fatfs_open ...
 *
 * 3. 磁盘驱动能实现对指定扇区的读写, 但是不能实现仅对特定文件读写
 */
