 ...
 * 异常/中断
 * 1. 概念
 *    系统正常运行时, 偶尔会有外部事件打断当前正在运行的程序(外部事件优先级更高), 或CPU内部产生的事件
 *    此时需要让该事件紧急执行, 执行完后再返回给当前被打断的程序继续运行
 *
 * 2. 分类
 * 1) 异常
 ...
 *          |  触发中断
 *          --- 保存中断上下文 ---
 *          ---------- 中断号 --- IDTR/中断向量表IDT --- 取出段选择因子/offset ----> 
 *          --- 段选择因子 ---> GDTR/全局描述符表GDT --- 取出base addr         ----> 中断处理程序入口地址
 *          --- 恢复中断上下文 ---
 *          当中断处理完成后, 返回正常程序
 * 中断发生前 |
 * 
 ...
 *
 * 当某一中断触发时的工作流程
 * 0) 中断触发前程序正常运行, 触发外部中断事件后进入中断处理流程
 * 1) 先通过IDTR寄存器得到内存中IDT表基地址 IDT Base Addres
